...
const Appointment = () => {
  ...
  const getAvailableSlot = async () => {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      setDocSlots((prev) => [...prev, { date: workingDate, slots: timeSlots }]);
    }
          ...
          <div className="flex gap-3 items-center w-full overflow-x-scroll mt-4">
            {docSlots.length &&
              docSlots.map((item, index) => (
                ...
                >
                  <p>{item && daysOfWeek[item.date.getDay()]}</p>
                  <p>{item && item.date.getDate()}</p>
                </div>
              ))}
          </div>
          <div className="flex items-center gap-3 w-full overflow-x-scroll mt-4">
            {docSlots.length && docSlots[slotIndex].slots.length > 0 ? (
              ...
              <p className="py-2 text-gray-800">No Slots Today</p>
            )}
          </div>
          <button
